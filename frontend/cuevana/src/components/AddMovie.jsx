...
function AddMovie() {
  ...
  async function actualizarEstado(
    ...
    if (estado.length > 0) {
      const confirmars = await confirmar(
        `Ya existe un csv: ${tipo}, ¿Desea sobreescribirlo?`
      );
      if (confirmars) {
        setEstado(jsonArray);
    ...
    <Container fluid="md" className="contenedor-addmovie">
      ...
      >
        <Toast.Header>
          ...
          <strong className="me-auto">CUEVANA</strong>
          <small>Cerrar</small>
        </Toast.Header>
        <Toast.Body>Se han cargado los csv a la DB correctamente</Toast.Body>
      </Toast>
      <Row>
        <Col>
          ...
          <Form.Group className="mb-5">
            ...
            <div className="d-flex">
              ...
              recursos.length > 0 ? (
                ...
                <DropdownButton
                  title="Escoger CSV a cargar"
                  id="bg-nested-dropdown"
                >
                  <Dropdown.Item
                    eventKey="actores"
                    onClick={(e) => cargarCSV("actores")}
                  >
                    Cargar Actores
                  </Dropdown.Item>
                  <Dropdown.Item
                    eventKey="peliculas"
                    onClick={(e) => cargarCSV("peliculas")}
                  >
                    Cargar Peliculas
                  </Dropdown.Item>
                  <Dropdown.Item
                    eventKey="reparto"
                    onClick={(e) => cargarCSV("reparto")}
                  >
                    Cargar Reparto
                  </Dropdown.Item>
                  <Dropdown.Item
                    eventKey="recursos"
                    onClick={(e) => cargarCSV("recursos")}
                  >
                    Cargar Recursos
                  </Dropdown.Item>
                </DropdownButton>
      ...
      <Row>
        {peliculas.length > 0 ? (
          <Tabla data={peliculas} tipo={"Peliculas"} />
        ) : null}
      </Row>
      <Row>
      ...
      <Row>
        {recursos.length > 0 ? (
          <Tabla data={recursos} tipo={"Recursos"} />
        ) : null}
      </Row>
    </Container>
  );
